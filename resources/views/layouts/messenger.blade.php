<img src="{{ asset('images/messenger.png') }}" alt="messenger" class="messenger" id="messenger">
<div class="messenger-box" id="messenger-box">
    <img src="{{ asset('images/close_icon.png') }}" alt="icon" class="icon-close">
    <p class="txt-name">HapoLearn</p>
    <div class="main-chat">
        <img src="{{ asset('images/ava-powl.png') }}" alt="owl" class="ava-owl">
        <p class="txt-chat">HapoLearn xin chào bạn.<br>Bạn có cần chúng tôi hỗ trợ gì không?</p>
    </div>
    <p class="txt-login">
        <i class="fab fa-facebook-messenger fa-lg"></i>
        Đăng nhập bằng messenger
    </p>

    <p class="chat-with-hapo">Chat với HapoLearn trong Messenger</p>
</div>
